...
@MainActor
class DiarySecurityManager: ObservableObject {
...
class SecurityAuditLogger {
    ...
    enum SecurityEvent {
        ...
        case authenticationFailure(Int)
        case authenticationAttemptWhileLocked
        case authenticationError(String)
        case manualLock
        case autoLock
        case accountLocked
        case lockoutExpired
        case securityLevelChanged(String)
        case integrityCheckFailed(String)
        case emergencyWipeInitiated
        case emergencyWipeCompleted
        case dataAccessed(String)
        case dataModified(String)
        case privacyViolationDetected(String)
        
        var description: String {
            switch self {
            case .systemInitialized:
                return "Sistema de segurança inicializado"
            case .authenticationSuccess:
                return "Autenticação bem-sucedida"
            case .authenticationFailure(let attempts):
                return "Falha na autenticação (tentativa \(attempts))"
            case .authenticationAttemptWhileLocked:
                return "Tentativa de autenticação durante bloqueio"
            case .authenticationError(let error):
                return "Erro na autenticação: \(error)"
            case .manualLock:
                return "Bloqueio manual ativado"
            case .autoLock:
                return "Bloqueio automático ativado"
            case .accountLocked:
                return "Conta bloqueada por tentativas excessivas"
            case .lockoutExpired:
                return "Bloqueio expirado"
            case .securityLevelChanged(let level):
                return "Nível de segurança alterado para \(level)"
            case .integrityCheckFailed(let error):
                return "Falha na verificação de integridade: \(error)"
            case .emergencyWipeInitiated:
                return "Limpeza de emergência iniciada"
            case .emergencyWipeCompleted:
                return "Limpeza de emergência concluída"
            case .dataAccessed(let type):
                return "Dados acessados: \(type)"
            case .dataModified(let type):
                return "Dados modificados: \(type)"
            case .privacyViolationDetected(let details):
                return "Violação de privacidade detectada: \(details)"
            }
        }
        
        var severity: LogSeverity {
            switch self {
            case .systemInitialized, .authenticationSuccess, .manualLock, .autoLock, .lockoutExpired:
                return .info
            case .authenticationFailure, .securityLevelChanged, .dataAccessed, .dataModified:
                return .warning
            case .authenticationAttemptWhileLocked, .authenticationError, .accountLocked, .integrityCheckFailed:
                return .error
            case .emergencyWipeInitiated, .emergencyWipeCompleted, .privacyViolationDetected:
                return .critical
            }
        }
    }
    ...
    func getRecentLogs(days: Int) async -> [SecurityAuditLog] {
        let startDate = Calendar.current.date(byAdding: .day, value: -days, to: Date()) ?? Date()
        return await storage.getLogs(since: startDate)
    }
...
struct SecurityAuditLog: Identifiable, Codable {
    let id: UUID
    let timestamp: Date
    let event: String
...
struct PrivacyComplianceReport: Codable {
    let score: Double
