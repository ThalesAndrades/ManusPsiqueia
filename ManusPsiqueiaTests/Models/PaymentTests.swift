...
class PaymentTests: XCTestCase {
    ...
    func testPaymentInitialization() {
        let payment = Payment(id: "pay_123", userId: "user_456", amount: 89.90, currency: "BRL", date: Date(), status: .succeeded)
        ...
        XCTAssertEqual(payment.status, .succeeded)
    }
    ...
    func testPaymentEncodingDecoding() throws {
        let originalPayment = Payment(id: "pay_123", userId: "user_456", amount: 89.90, currency: "BRL", date: Date(), status: .succeeded)
    ...
    func testPaymentValidation() {
        ...
        let validPayment = Payment(id: "pay_123", userId: "user_456", amount: 89.90, currency: "BRL", date: Date(), status: .pending)
        XCTAssertGreaterThan(validPayment.amount, 0)
        XCTAssertFalse(validPayment.currency.isEmpty)
        XCTAssertFalse(validPayment.id.isEmpty)
        XCTAssertFalse(validPayment.userId.isEmpty)
    }
}
