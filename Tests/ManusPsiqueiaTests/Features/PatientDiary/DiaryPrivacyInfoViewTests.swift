import XCTest
import SwiftUI
@testable import ManusPsiqueia

class DiaryPrivacyInfoViewTests: XCTestCase {

    func testDiaryPrivacyInfoViewInitialization() {
        // Similar to other SwiftUI Views, direct unit testing of its UI behavior
        // is best done with SwiftUI Test or Snapshot Testing.
        // Here, we ensure basic initialization without crashing.
        
        let view = DiaryPrivacyInfoView()
        XCTAssertNotNil(view, "DiaryPrivacyInfoView should be initializable")
    }
}


